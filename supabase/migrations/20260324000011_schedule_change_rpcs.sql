-- Migration: RPCs cho schedule change với reason + deadline lock enforcement
-- Story 2.3: update_slot_with_reason + delete_slot_with_reason
--
-- Lý do cần RPC (không direct client call):
--   - Atomic: update/delete slot + audit trail + notifications trong 1 transaction
--   - Server-side deadline lock enforcement (client-side chỉ là UX)
--   - Ownership validation: slot phải thuộc về caller
--   - SECURITY DEFINER: cho phép notify managers mà không bị RLS user_id check block

-- ================================================================
-- RPC 1: update_slot_with_reason
-- Cập nhật một slot với lý do bắt buộc. Hỗ trợ emergency override cho locked slots.
-- ================================================================
...
BEGIN
  ...
  -- Validate duration range (30–720 phút) — defense-in-depth cho direct API callers
  ...
  -- Guard NULL timezone — fallback về UTC nếu tenant chưa cấu hình
  ...
  -- Audit trail: ghi change record (tồn tại vĩnh viễn vì slot vẫn còn)
  ...
  -- Notify tất cả managers/owners trong tenant
  -- Không self-notify (nếu member cũng có role manager/owner)
...
-- Xóa một slot với lý do bắt buộc. Hỗ trợ emergency override cho locked slots.
--
-- NOTE CASCADE limitation:
--   schedule_slot_changes.slot_id có ON DELETE CASCADE.
--   Khi slot bị DELETE, mọi change record của slot đó cũng bị xóa.
--   Do đó: KHÔNG insert change record cho delete (vô nghĩa — bị cascade ngay).
--   Notifications tới managers đóng vai trò audit trail cho deletion (message chứa lý do).
...
BEGIN
  ...
  -- Xóa slot (CASCADE sẽ xóa schedule_slot_changes của slot này — acceptable cho MVP)
